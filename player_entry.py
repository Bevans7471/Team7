    ...
    def get_players(self):
        ...
        for id_slot, name_slot in self.slots:
            if id_slot.text.strip() and name_slot.text.strip():
                ...
                if equipment_id:  #include if they have equipment assigned
                    players.append({
                        'player_id': player_id,
                        'codename': codename,
...
# Handle keyboard events (F5 = start, F12 = clear)

    # Get players from both teams
    red_players = red_team.get_players()
    green_players = green_team.get_players()

    print(f"[GAME] Red team: {len(red_players)} players")
    print(f"[GAME] Green team: {len(green_players)} players")
    ...
    while running:
        ...
        for player in red_players:
            player_text = SMALL_FONT.render(
                f"{player['player_id']}: {player['codename']} [Equip: {player['equipment_id']}]",
                True, WHITE
            )
            ...
            if event.type == pygame.QUIT:
                ...
                # F12 key to clear
                    ...
                    if event.key == pygame.K_RETURN:
                        ...
                        #Save codename to database (without hardware_id)
                        if player_id and codename:
                            save_player(player_id, codename)

                        #Store equipment ID locally for this game only
                        if equipment_id:
                            try:
                                equip_id_int = int(equipment_id)
                                player_equipment[player_id] = equip_id_int

                                #Broadcast equipment ID via UDP immediately
                    ...
                    for team in [red_team, green_team]:
                        ...
                        if result == "ID_ENTERED":
                            print("[INFO] Processing player entry...")
                            player_id = id_slot.text
                            ...
                            print("[INFO] Ready for next player input.")
